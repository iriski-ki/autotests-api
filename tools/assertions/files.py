...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
        Проверяет, что тело ответа с ошибкой валидации при передаче невалидного UUID
        соответствует ожидаемой структуре и тексту ошибки API.

        :param actual: Фактический ответ API, преобразованный в ValidationErrorResponseSchema.
        :raises AssertionError: Если структура или значения ошибки не совпадают.
        """
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
