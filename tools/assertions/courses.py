from clients.courses.courses_schema import UpdateCourseRequestSchema, UpdateCourseResponseSchema,CourseSchema,CreateCourseResponseSchema,GetCoursesResponseSchema
...
def assert_create_course_response(
    ...
    """
    Проверяет, что поля созданного курса соответствуют данным из запроса на создание.

    :param request: Данные запроса на создание курса.
    :param response: Ответ API с созданным курсом.
    :raises AssertionError: Если поля курса не совпадают.
    """
    assert_equal(response.course.title, request.title, "title")
    assert_equal(response.course.max_score, request.max_score, "max_score")
    assert_equal(response.course.min_score, request.min_score, "min_score")
